...
<div class="container plugin-shop">
    <div class="row">
                    ...
                    <?php foreach ($search_sections as $v) { ?>
                        <li class="dropdown" style="padding-bottom: 0px;padding-top: 0px;">
                            ...
                            <?php foreach ($search_categories_section[$v['Section']['id']] as $va) { ?>
                                <li><a href="<?= $this->Html->url(['controller' => 'c/'.$va['Category']['id'], 'plugin' => 'shop']) ?>"><?= before_display($va['Category']['name']) ?></a></li>
                            <?php } ?>
                            </ul>
                        </li>
                    <?php } ?>
                    <?php foreach ($search_categories_without_section as $v) { ?>
                        <a href="<?= $this->Html->url(['controller' => 'c/'.$v['Category']['id'], 'plugin' => 'shop']) ?>" class="list-group-item<?= (isset($category) AND $v['Category']['id'] == $category OR !isset($category) AND $i == 1) ? ' active' : ''; ?>"><?= before_display($v['Category']['name']) ?></a>
                    <?php } ?>
                </ul>
            </div>
      ...
      <div class="col-xs-12 col-md-9">
            <div class="row"><?= $vouchers->get_vouchers() // Current promotions ?></div>
            <div class="row">
            <?php
                $col = 4;
                $i = 0;
                        ...
                        <div class="col-sm-<?= $col ?> col-lg-<?= $col ?> col-md-<?= $col ?>">
                            <div class="thumbnail">
                                ...
                                <div class="caption" style="height:auto;">
                                    ...
                                    <h4><a href="#"><?= before_display($v['Item']['name']) ?></a></h4>
                                    <p><?= $this->Text->truncate(strip_tags($v['Item']['description']), 140, ['ellipsis' => '...', 'html' => true]) ?></p>
...
<script type="text/javascript">
    ...
    var ITEM_GET_URL = '<?= $this->Html->url(['controller' => 'shop/ajax_get', 'plugin' => 'shop']); ?>/';
    var VOUCHER_CHECK_URL = '<?= $this->Html->url(['action' => 'checkVoucher']) ?>/';
    var BUY_URL = '<?= $this->Html->url(['action' => 'buy_ajax']) ?>';
